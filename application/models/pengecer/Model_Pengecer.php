...
class Model_Pengecer extends CI_Model {
    ...
    public function get($id=null){
        ...
        SELECT 
                ...
                INNER JOIN login ON pengecer.id_login = login.id_login
    ...
    public function jumlahMasuk($id=null){
        $query = $this->db->query("
        SELECT 
                pupuk_terima.pterima_id,
                pupuk_terima.tahun_id,
    ...
    public function by_tahun($tahun){
        ...
        SELECT 
                ...
                INNER JOIN distributor ON pupuk_terima.distributor_id = distributor.distributor_id
                INNER JOIN login ON pengecer.id_login = login.id_login
                WHERE pupuk_terima.tahun_id= $tahun AND login.id_login = '".$_SESSION['id_login']."'
                ORDER BY nama_bulan
        ");
    if($query->num_rows() > 0){
        foreach($query->result() as $data){
    ...
    $this->db->join('login', 'pengecer.id_login = login.id_login');
    $this->db->order_by('nama_pengecer', 'asc');
    $this->db->group_by('nama_pengecer');
